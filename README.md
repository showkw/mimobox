...
| microVM 级 | 已完成首版 | 基于 Linux KVM，已打通 guest `/init` + 串口命令协议 + snapshot/restore，并支持流式输出、文件传输和 HTTP 代理等 guest/host 控制面能力 |
...
| 命令级环境变量注入与超时 | 已完成 | `execute_with_env()`、`execute_with_timeout()`、`execute_with_env_and_timeout()` 已落地；当前仅支持 Linux + microVM 后端 |
...
| 结构化错误模型 | 已完成 | Rust 侧已提供 `ErrorCode` + `SdkError::Sandbox { code, message, suggestion }`，Python 侧映射为 `SandboxProcessError` / `SandboxHttpError` / `SandboxLifecycleError` 等异常层级 |
| 预热池 | 已完成 | `mimobox-os` 与 `mimobox-vm` 均提供预热池，OS 级 P50 0.19us（acquire only），microVM 热路径 P50 773us |
| microVM Guest 执行链路 | 已完成首版 | guest `/init` 会输出 `READY`，host 可发送 `EXEC:`、`EXECS:`、`HTTP:REQUEST:` 等命令帧，guest 回传 `OUTPUT:` / `EXIT:`、`STREAM:*`、`HTTPRESP:*` |
...
> **口径说明**：以下为 2026-04-22 在 Intel Xeon E5-2686 v4 @ 2.30GHz 上的内部轻载基准（/bin/echo、/bin/true），不代表真实业务端到端延迟。microVM benchmark 需 `--features kvm` 启用。

| 后端 | 指标 | 目标 | P50 | P95 | P99 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| OS 级 | 冷启动 | <10ms | 8.24ms | 8.81ms | 8.99ms | ✅ |
...
| microVM (KVM) | 预热池热路径 | <1ms | 773us | 782us | 793us | ✅ |

*\* OS 预热池数字为 acquire()+drop() 对象获取成本，不包含命令执行。*
*\*\* 池化快照恢复为 restore-to-ready（从 RestorePool 取出空壳 VM + 写入 memory + 恢复 vCPU 状态），不含命令执行和池补充开销。非池化快照恢复含全生命周期。*
...
- `execute_with_env()`、`execute_with_timeout()`、`execute_with_env_and_timeout()` 当前仅支持 Linux + `vm` feature 的 microVM 后端
...
- `stream_execute()` 当前仅支持 Linux + `vm` feature 的 microVM 后端
- 该接口适合长时间命令、长日志编译、安装过程和需要边执行边消费输出的 Agent 任务
...
- `http_request()` 当前仅支持 Linux + `vm` feature 的 microVM 后端
- `allowed_http_domains` 是 host HTTP 代理层的域名白名单，不等价于“沙箱内任意网络已开放”
- 当前仅允许 HTTPS，请求超时、域名拒绝和 URL 非法等失败会映射到结构化错误码
