...
- **Agent-native** — 内置包含 15 个 tools 的 MCP server、Python SDK，并开箱支持 LangChain / OpenAI Agents SDK 集成。

各隔离层的详细性能数据见[性能](#性能)。

## 核心特性

- **多层隔离** — OS（Landlock + Seccomp + Namespaces）、Wasm（Wasmtime）和 microVM（KVM），支持智能自动路由
- **沙箱注册表** — `Sandbox.list()` 追踪所有活跃实例，`Sandbox.id` 提供唯一标识
- **持久环境变量** — 通过 `env_vars` 在沙箱创建时设置环境变量，对所有后续命令生效
- **运行时指标** — 使用 `Sandbox.metrics()` 监控每个沙箱的 CPU、内存、I/O 和 Wasm fuel
- **MCP Server** — 15 个工具支持 AI agent 集成，包括文件管理和 HTTP ACL
- **超低延迟** — OS 冷启动 P50 8.24 ms，Wasm 冷启动 P50 1.01 ms，warm pool acquire P50 0.19 µs
